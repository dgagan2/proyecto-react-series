...
const Main = () => {
  ...
  function deleteTextFromString (id, urlImage, name, texto, urlSerie) {
    if (state) {
      ...
      const summary = deleteText.replace(/<\/?p>|<\/?b>|p>/g, '')
      return <Link key={id} to={`/serie/${id}`} style={{ color: 'white' }} className='description' onMouseLeave={() => setState(false)}>{summary}...</Link>
    } else {
